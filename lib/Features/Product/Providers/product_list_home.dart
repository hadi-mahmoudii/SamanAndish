...
class ProductListHomeProvider extends ChangeNotifier with ReassembleHandler {
  ...
  ProductListHomeProvider(this.parentCategory);
  ...
  getDatas(BuildContext context, {bool resetPage = false}) async {
    ...
    final Either<ErrorResult, dynamic> result = await ServerRequest().fetchData(
      Urls.getProductsHome(parentCategory.id, currentPage.toString()),
    );
    result.fold(
      (error) async {
